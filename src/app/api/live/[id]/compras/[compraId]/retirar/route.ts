...
export async function DELETE(req: NextRequest, { params }: Params) {
  ...
  if (!compra) return NextResponse.json({ erro: "Compra não encontrada." }, { status: 404 })
  if (compra.status_compra !== "retirada")
    return NextResponse.json({ erro: "Compra não está com status retirada." }, { status: 422 })

  const { error } = await sb
    .from("live_compras")
    .update({ status_compra: "finalizada" })
    .eq("id", parseInt(compraId))

  if (error) {
    console.error("[DELETE retirar]", error)
    return NextResponse.json({ erro: error.message }, { status: 500 })
  }
  return NextResponse.json({ ok: true })
}
