...
// Fotos de celular chegam a 12MB; reduz para ~2800px JPEG (letra manuscrita
...
export default function ImportarPorFoto({
  liveId,
  liveData,
  onClose,
  onSalvo,
  ...
  async function aoEscolherArquivo(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.type.startsWith("image/")) {
      setErroMsg("Escolha um arquivo de imagem.");
      return;
    }
    try {
  ...
  const analisar = useCallback(async () => {
    ...
    setErroMsg("");
    try {
      const res = await apiPost<RespostaAnalise>(
        `/live/${liveId}/importar-foto`,
        { imagem },
      );
      ...
      setCompras(
        res.compras.map((c) => ({
          ...
          quantidade_itens:
            c.quantidade_itens != null ? String(c.quantidade_itens) : "",
          valor_total: numParaInput(c.valor_total),
          itens_lidos: c.itens_lidos ?? [],
          observacao: c.observacao ?? "",
          confianca: c.confianca ?? {},
          sugestoes: c.sugestoes ?? [],
          // vincula automaticamente apenas match 100%
          clienteSel:
            c.sugestoes?.[0]?.match === "exato" ? c.sugestoes[0] : null,
        })),
      );
      setFase("revisao");
    } catch (e) {
      ...
      cs.map((c) =>
        c.key === key
          ? {
              ...
              clienteSel: s,
              ...(s
                ? { nome_cliente: s.nome, whatsapp: s.celular ?? c.whatsapp }
                : {}),
            }
          : c,
      ),
  ...
  async function salvarTudo() {
    ...
    const invalidas = pendentes.filter(
      (c) => !c.nome_cliente.trim() || !c.valor_total.trim(),
    );
    if (invalidas.length > 0) {
      setErroMsg(
        `${invalidas.length} compra(s) sem nome ou valor. Complete os campos destacados.`,
      );
      return;
    }
    setErroMsg("");
    ...
    for (let i = 0; i < pendentes.length; i++) {
      ...
      try {
        ...
        const credito = c.clienteSel
          ? Math.min(Number(c.clienteSel.saldo_credito ?? 0), valor)
          : 0;
    ...
    if (erros === 0) {
      ...
      setErroMsg(
        `${erros} compra(s) não foram salvas. Revise os cards com erro e tente novamente.`,
      );
      setFase("revisao");
    }
  }
  ...
  function bordaConfianca(
    ...
    const vazio = !(c[campo as keyof CompraRevisao] as string)
      ?.toString()
      .trim();
                ...
                {!imagem ? (
                  <>
                    <div className="text-center mb-8">
                      ...
                      >
                        Tire uma foto da página com as anotações das compras. O
                        sistema identifica clientes, sacolas e valores
                        automaticamente.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <motion.button
                        ...
                        style={{
                          background: `linear-gradient(135deg, ${COR_LIVE}, #9f1239)`,
                        }}
                      >
                        <Camera size={32} />
                        <div>
                    ...
                    >
                      ...
                      >
                        ...
                        boa iluminação, página inteira visível e sem sombras
                        melhoram muito a leitura. Você revisa tudo antes de
                        salvar.
                      </p>
                    </div>
                  </>
                  ...
                  <>
                    ...
                    <p
                      ...
                      style={{ color: "var(--text-muted)" }}
                    >
                      A página está nítida e bem enquadrada?
                    </p>
                    <motion.div
            ...
            >
                  ...
                  {compras.map((c, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2 min-w-0">
                          <span
                            ...
                            style={{
                              background: c.salva ? "#22c55e" : COR_LIVE,
                            }}
                          >
                      ...
                      {!c.salva && c.sugestoes.length > 0 && !c.clienteSel && (
                        ...
                        >
                          <p
                            ...
                            style={{ color: "#d97706" }}
                          >
                            <UserCheck size={12} /> Cliente parecida no cadastro
                            — é uma destas?
                          </p>
                      ...
                      {!c.salva && c.clienteSel && (
                        <div
                          className="mb-3 px-3 py-2 rounded-xl flex items-center justify-between gap-2 flex-wrap"
                          style={{
                            background: "#22c55e10",
                          ...
                          >
                            ...
                            {c.clienteSel.saldo_credito > 0 && (
                              <span
                                ...
                                style={{ color: "#d97706" }}
                              >
                                <Wallet size={12} /> Crédito{" "}
                                {fmtBRL(
                                  Math.min(
                                    c.clienteSel.saldo_credito,
                                    inputParaNum(c.valor_total) ||
                                      c.clienteSel.saldo_credito,
                                  ),
                                )}{" "}
                                será abatido
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                        <div className="col-span-2">
                          ...
                          <input
                            ...
                            disabled={c.salva}
                            onChange={(e) =>
                              setCampo(c.key, "nome_cliente", e.target.value)
                            }
                            ...
                            style={{
                              ...iSt,
                              borderStyle: "solid",
                              ...bordaConfianca(c, "nome_cliente", true),
                            }}
                          />
                        </div>
                        <div className="col-span-2">
                          ...
                          <input
                            ...
                            disabled={c.salva}
                            onChange={(e) =>
                              setCampo(c.key, "whatsapp", e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <input
                            value={c.numero_sacola}
                            disabled={c.salva}
                            onChange={(e) =>
                              setCampo(c.key, "numero_sacola", e.target.value)
                            }
                            placeholder="—"
                            inputMode="numeric"
                            className="w-full px-3 py-2.5 text-sm rounded-xl outline-none"
                            style={{
                              ...iSt,
                              borderStyle: "solid",
                              ...bordaConfianca(c, "numero_sacola"),
                            }}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            style={{
                              ...iSt,
                              borderStyle: "solid",
                              ...bordaConfianca(c, "quantidade_itens"),
                            }}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            style={{
                              ...iSt,
                              borderStyle: "solid",
                              ...bordaConfianca(c, "valor_total", true),
                            }}
                          />
                        </div>
                        <div className="col-span-2 sm:col-span-4">
                          {c.itens_lidos.some((item) => item !== null) && (
                            ...
                            >
                              ...
                              {c.itens_lidos.map((item, index) =>
                                item === null ? null : (
                                  <span key={index} className="mr-2">
                                    I{index + 1}: {fmtBRL(item)}
                                  </span>
                                ),
                              )}
                            </div>
                          ...
                          <input
                            ...
                            disabled={c.salva}
                            onChange={(e) =>
                              setCampo(c.key, "observacao", e.target.value)
                            }
                      ...
                      {!c.salva &&
                        Object.values(c.confianca ?? {}).some(
                          ...
                          >
                            ...
                            Alguns campos ficaram com leitura duvidosa — confira
                            os destacados.
                          </p>
                        )}
                      {c.erroSalvar && (
              ...
              >
                <div className="max-w-3xl mx-auto flex items-center gap-3">
                  ...
                  >
                    <Check size={15} /> Salvar {pendentes} Compra
                    {pendentes !== 1 ? "s" : ""}
                  </motion.button>
                </div>
              </div>
          ...
          {fase === "concluido" && (
            ...
            >
              ...
              <p
                ...
                style={{ color: "var(--text-muted)" }}
              >
                {progresso.total} compra
                {progresso.total !== 1 ? "s foram salvas" : " foi salva"} na
                live. Agora você pode vincular os produtos de cada uma.
              </p>
              <div className="flex flex-col sm:flex-row gap-3">
                <motion.button
                  whileHover={{ scale: 1.02 }}
