...
// Prints de celular chegam grandes; preserva até ~2600px para manter letras pequenas.
...
export default function ImportarClientePorFoto({
  onClose,
  onSalvo,
}: {
  onClose: () => void;
  ...
  useEffect(() => {
    ...
    const t = setInterval(
      () => setMsgIdx((i) => (i + 1) % MSGS_PROCESSANDO.length),
      2600,
    );
  ...
  async function aoEscolherArquivos(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (soImagens.length === 0) {
      setErroMsg("Escolha arquivos de imagem.");
      return;
    }
    try {
      const novas: string[] = [];
      ...
      setImagens((prev) => {
        ...
        if (juntas.length > MAX_PRINTS)
          setErroMsg(
            `Máximo de ${MAX_PRINTS} prints por análise — os extras foram ignorados.`,
          );
        else setErroMsg("");
  ...
  const analisar = useCallback(async () => {
    ...
    setErroMsg("");
    try {
      const res = await apiPost<RespostaAnalise>("/clientes/importar-foto", {
        imagens,
      });
      if (!res.legivel || res.clientes.length === 0) {
        setErroMsg(
          res.motivo ?? "Não encontrei dados cadastrais nestes prints.",
        );
        setFase("erro");
        return;
      }
      setClientes(
        res.clientes.map((c) => ({
          ...
          imagens_origem: c.imagens_origem ?? [],
      ...
      setErroMsg(
        e instanceof Error ? e.message : "Erro ao analisar os prints.",
      );
      setFase("erro");
    }
  }, [imagens]);
  ...
  function setCampo(key: number, campo: keyof ClienteRevisao, valor: string) {
    setClientes((cs) =>
      cs.map((c) => {
        ...
        if (campo in (c.confianca ?? {}))
          upd.confianca = { ...c.confianca, [campo]: "alta" };
        return upd;
      }),
    );
  }
  ...
  async function salvarTudo() {
    ...
    if (invalidos.length > 0) {
      setErroMsg(
        `${invalidos.length} cadastro(s) sem nome. Complete os campos destacados.`,
      );
      return;
    }
    const inconsistentes = pendentes.filter((c) => {
      const celular = c.celular.replace(/\D/g, "");
      const cep = c.cep.replace(/\D/g, "");
      return (
        (c.cpf_cnpj.trim() !== "" && !isValidCpfCnpj(c.cpf_cnpj)) ||
        (c.data_nasc.trim() !== "" && !normalizeBirthDate(c.data_nasc)) ||
        (c.email.trim() !== "" && !isValidEmail(c.email.trim())) ||
        (celular !== "" && ![10, 11].includes(celular.length)) ||
        (cep !== "" && cep.length !== 8)
      );
    });
    if (inconsistentes.length > 0) {
      setErroMsg(
        `${inconsistentes.length} cadastro(s) têm CPF/CNPJ, data, e-mail, telefone ou CEP inválido. Corrija o campo destacado ou deixe-o vazio.`,
      );
      return;
    }
    setErroMsg("");
    ...
    for (let i = 0; i < pendentes.length; i++) {
      ...
      try {
        ...
        setClientes((cs) =>
          cs.map((x) =>
            x.key === c.key ? { ...x, salva: true, erroSalvar: undefined } : x,
          ),
        );
      } catch (e) {
        erros++;
        ...
        setClientes((cs) =>
          cs.map((x) => (x.key === c.key ? { ...x, erroSalvar: msg } : x)),
        );
      }
    }
    ...
    if (erros === 0) {
      ...
      setErroMsg(
        `${erros} cadastro(s) não foram salvos. Revise os cards com erro e tente novamente.`,
      );
      setFase("revisao");
    }
  }
  ...
  function bordaConfianca(
    c: ClienteRevisao,
    campo: string,
    obrigatorio = false,
  ): { borderColor: string; borderWidth: number } {
    const vazio = !(c[campo as keyof ClienteRevisao] as string)
      ?.toString()
      .trim();
    if (obrigatorio && vazio) return { borderColor: "#ef4444", borderWidth: 2 };
    const valor = String(c[campo as keyof ClienteRevisao] ?? "").trim();
    const digitos = valor.replace(/\D/g, "");
    const invalido =
      (campo === "cpf_cnpj" && valor !== "" && !isValidCpfCnpj(valor)) ||
      (campo === "data_nasc" && valor !== "" && !normalizeBirthDate(valor)) ||
      (campo === "email" && valor !== "" && !isValidEmail(valor)) ||
      (campo === "celular" &&
        digitos !== "" &&
        ![10, 11].includes(digitos.length)) ||
      (campo === "cep" && digitos !== "" && digitos.length !== 8);
    if (invalido) return { borderColor: "#ef4444", borderWidth: 2 };
    const nivel = c.confianca?.[campo];
    if (!vazio && nivel === "baixa")
      return { borderColor: "#ef4444", borderWidth: 2 };
    if (!vazio && nivel === "media")
      ...
      <div className={opts?.span ?? ""}>
        ...
        <input
          ...
          style={{
            ...iSt,
            borderStyle: "solid",
            ...bordaConfianca(c, campo, opts?.obrigatorio),
          }}
        />
      </div>
    );
      ...
      <div className="flex-1 overflow-y-auto">
        <AnimatePresence mode="wait">
              ...
              <div className="w-full max-w-xl">
                {imagens.length === 0 ? (
                  <>
                    <div className="text-center mb-8">
                      ...
                      >
                        Envie prints do WhatsApp ou Instagram com os dados
                        cadastrais (nome, telefone, endereço...). O sistema lê e
                        preenche o cadastro para você revisar.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <motion.button
                        ...
                        style={{
                          background: `linear-gradient(135deg, ${COR}, ${COR_ESCURA})`,
                        }}
                      >
                        <Camera size={32} />
                        <div>
                    ...
                    >
                      ...
                      >
                        ...
                        você pode enviar até {MAX_PRINTS} prints de uma vez —
                        por exemplo, o perfil do WhatsApp e a mensagem com o
                        endereço. Prints da mesma pessoa viram um único
                        cadastro. Você revisa tudo antes de salvar.
                      </p>
                    </div>
                  </>
                ) : (
                  <>
                    <h1
                      ...
                      style={{ color: "var(--text-primary)" }}
                    >
                      {imagens.length} print{imagens.length !== 1 ? "s" : ""}{" "}
                      selecionado{imagens.length !== 1 ? "s" : ""}
                    </h1>
                    <p
                      className="text-sm mb-5 text-center"
                      style={{ color: "var(--text-muted)" }}
                    >
                      Os dados estão legíveis nas capturas?
                    </p>

                    <div
                      className={`grid gap-3 mb-5 ${imagens.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}
                    >
                      {imagens.map((img, idx) => (
                        <motion.div
                          key={idx}
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      ...
                      >
                        <Sparkles size={15} /> Analisar Print
                        {imagens.length !== 1 ? "s" : ""}
                      </motion.button>
                    </div>
                  </>
                ...
                {erroMsg && (
                  <motion.p
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="mt-4 text-sm text-center flex items-center justify-center gap-1.5"
                    style={{ color: "#ef4444" }}
                  >
          ...
          {fase === "processando" && (
            ...
            >
              <div
                className="relative w-40 h-52 rounded-2xl overflow-hidden mb-8"
                style={{ border: "1px solid var(--border)" }}
              >
                {imagens[0] && (
                  // eslint-disable-next-line @next/next/no-img-element
            ...
            >
              ...
              <div className="mb-5">
                ...
                <p className="text-sm" style={{ color: "var(--text-muted)" }}>
                  Revise e corrija antes de salvar. Nada é salvo sem a sua
                  confirmação.
                </p>
              ...
              <div className="space-y-4">
                <AnimatePresence>
                  {clientes.map((c, idx) => {
                    const jaCadastrada = c.sugestoes.find(
                      (s) => s.match === "exato",
                    );
                    const parecidas = c.sugestoes.filter(
                      (s) => s.match === "parecido",
                    );
                    return (
                      <motion.div
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2 min-w-0">
                            <span
                              className="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-black shrink-0 text-white"
                            ...
                            {jaCadastrada && !c.salva ? (
                              <span
                                className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide px-2 py-1 rounded-full truncate"
                                style={{
                                  background: "#f59e0b1a",
                                  color: "#d97706",
                                }}
                        ...
                        {!c.salva && jaCadastrada && (
                          ...
                          >
                            <p
                              className="text-xs font-semibold flex items-center gap-1.5 flex-wrap"
                              style={{ color: "#d97706" }}
                            >
                        ...
                        {c.imagens_origem.length > 0 && !c.salva && (
                          <p
                            className="mb-3 text-[11px]"
                            style={{ color: "var(--text-muted)" }}
                          >
                            Dados comprovados no
                            {c.imagens_origem.length > 1 ? "s" : ""} print
                            {c.imagens_origem.length > 1 ? "s" : ""}{" "}
                            {c.imagens_origem.join(", ")}.
                          </p>
                        )}

                        {/* Campos — identificação */}
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                          {campoInput(c, "nome", "Nome Completo", {
                            obrigatorio: true,
                          ...
                          {campoInput(c, "celular", "WhatsApp", {
                            placeholder: "16999999999",
                            inputMode: "numeric",
                            span: "col-span-2 sm:col-span-1",
                          })}
                          {campoInput(c, "instagram", "Instagram", {
                            placeholder: "usuario",
                            span: "col-span-2 sm:col-span-1",
                          })}
                          {campoInput(c, "cpf_cnpj", "CPF", {
                            placeholder: "Somente números",
                            inputMode: "numeric",
                          })}
                          {campoInput(c, "data_nasc", "Nascimento", {
                            tipo: "date",
                          })}
                          {campoInput(c, "email", "E-mail", {
                        ...
                        {/* Campos — endereço */}
                        <p
                          className="text-[10px] font-bold uppercase tracking-wider mt-4 mb-2"
                          style={{ color: "var(--text-muted)" }}
                        >
                          Endereço
                        </p>
                        <div className="grid grid-cols-2 sm:grid-cols-6 gap-2.5">
                          {campoInput(c, "cep", "CEP", {
                            placeholder: "00000000",
                            inputMode: "numeric",
                          })}
                          {campoInput(c, "logradouro", "Rua / Av.", {
                            span: "col-span-2 sm:col-span-3",
                          })}
                          {campoInput(c, "numero", "Número")}
                          {campoInput(c, "complemento", "Complemento")}
                          {campoInput(c, "bairro", "Bairro", {
                            span: "sm:col-span-2",
                          })}
                          {campoInput(c, "cidade", "Cidade", {
                            span: "sm:col-span-2",
                          })}
                          {campoInput(c, "estado", "UF", {
                            placeholder: "SP",
                            upper: true,
                          })}
                        </div>
                        ...
                        {!c.salva &&
                          Object.values(c.confianca ?? {}).some(
                            (n) => n !== "alta",
                          ) && (
                            <p
                              className="mt-2.5 text-[11px] flex items-center gap-1.5"
                            ...
                            >
                              ...
                              Alguns campos ficaram com leitura duvidosa —
                              confira os destacados.
                            </p>
                          )}
                        {c.erroSalvar && (
                          <p
                            className="mt-2.5 text-[11px] flex items-center gap-1.5 font-semibold"
                            style={{ color: "#ef4444" }}
                          >
              ...
              <div
                className="fixed bottom-0 left-0 right-0 px-3 sm:px-6 py-3 sm:py-4"
                style={{
              ...
              >
                <div className="max-w-3xl mx-auto flex items-center gap-3">
                  {erroMsg ? (
                    <p
                      className="flex-1 text-xs font-semibold flex items-center gap-1.5 min-w-0"
                      style={{ color: "#ef4444" }}
                    >
                      <AlertTriangle size={13} className="shrink-0" />{" "}
                      <span className="truncate">{erroMsg}</span>
                    </p>
                  ) : (
                    <p
                      className="flex-1 text-xs hidden sm:block"
                      style={{ color: "var(--text-muted)" }}
                    >
                      {salvos > 0 && (
                        <span style={{ color: "#16a34a", fontWeight: 700 }}>
                          {salvos} salvo{salvos !== 1 ? "s" : ""} ·{" "}
                        </span>
                      )}
                      Confira os dados antes de confirmar. Clientes com WhatsApp
                      recebem a mensagem de consentimento automaticamente.
                    </p>
                  )}
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                  ...
                  >
                    <Check size={15} /> Salvar {pendentes} Cadastro
                    {pendentes !== 1 ? "s" : ""}
                  </motion.button>
                </div>
              </div>
          ...
          {fase === "salvando" && (
            ...
            >
              ...
              <div
                className="w-56 h-1.5 rounded-full mt-5 overflow-hidden"
          ...
          {fase === "concluido" && (
            ...
            >
              ...
              <p
                ...
                style={{ color: "var(--text-muted)" }}
              >
                {progresso.total} cliente
                {progresso.total !== 1
                  ? "s foram cadastrados"
                  : " foi cadastrado"}
                . Quem tem WhatsApp já recebe a mensagem de consentimento
                automaticamente.
              </p>
              <div className="flex flex-col sm:flex-row gap-3">
                <motion.button
