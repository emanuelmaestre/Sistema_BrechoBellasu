-- ══════════════════════════════════════════════════════════════════
-- 008 — VENDAS ATÔMICAS (corrige schema real + valida estoque)
-- Execute UMA vez no Supabase SQL Editor.
-- Idempotente: CREATE OR REPLACE pode rodar novamente sem efeitos.
--
-- POR QUE: as funções fn_criar_venda/fn_cancelar_venda originais (001)
-- referenciavam `produtos.estoque`, coluna que NÃO existe no banco real
-- (a coluna correta é `produtos.estoque_atual`). Logo o cancelamento de
-- venda estava quebrado e a criação atômica nunca era usada. Esta
-- migration recria ambas com as colunas reais e adiciona validação de
-- estoque com trava de linha (FOR UPDATE) para segurança em concorrência.
-- ══════════════════════════════════════════════════════════════════

-- ── Criar venda: insere venda + itens + baixa estoque, atomicamente ──
...
DECLARE
  ...
  v_nome     TEXT;
BEGIN
  -- 1ª passada: soma subtotal e valida estoque (trava as linhas dos produtos)
  ...
  -- 2ª passada: insere itens e baixa estoque (apenas produtos que controlam)
  FOR item IN SELECT * FROM jsonb_array_elements(p_itens) LOOP
    v_pid := NULLIF(item->>'produto_id','')::BIGINT;
    v_qtd := (item->>'qtd')::INT;
...
-- ── Cancelar venda: marca cancelada + estorna estoque (coluna correta) ──
...
-- ══════════════════════════════════════════════════════════════════
-- FIM — aplique antes de fazer deploy do código que usa estas funções.
-- ══════════════════════════════════════════════════════════════════
